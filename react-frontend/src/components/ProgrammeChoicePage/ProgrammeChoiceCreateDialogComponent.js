...
const ProgrammeChoiceCreateDialogComponent = (props) => {
    ...
    const validate = () => {
        ...
        const error = {};
            ...
            if (_.isEmpty(_entity?.intakeYear)) {
                error["intakeYear"] = `IntakeYear field is required`;
                ret = false;
            }
            ...
            if (_.isEmpty(_entity?.month)) {
                error["month"] = `Month field is required`;
                ret = false;
            }
  
            if (_.isEmpty(_entity?.campusLocation)) {
                error["campusLocation"] = `CampusLocation field is required`;
                ret = false;
            }
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            intakeYear: _entity?.intakeYear,courseName: _entity?.courseName,month: _entity?.month,campusLocation: _entity?.campusLocation,mode: _entity?.mode,
        ...
        const result = await client.service("programmeChoice").create(_data);
        props.onHide();
        props.alert({ type: "success", title: "Create info", message: "Info ProgrammeChoice created successfully" });
        props.onCreateResult(result);
        } catch (error) {
            console.log("error", error);
            setError(getSchemaValidationErrorsStrings(error) || "Failed to create");
            props.alert({ type: "error", title: "Create", message: "Failed to create in ProgrammeChoice" });
        }
        setLoading(false);
    };
    ...
    return (
        <Dialog header="Create ProgrammeChoice" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max" footer={renderFooter()} resizable={false}>
            <div className="grid p-fluid overflow-y-auto"
            style={{ maxWidth: "55vw" }} role="programmeChoice-create-dialog-component">
            ...
            {!_.isEmpty(error["intakeYear"]) ? (
              <p className="m-0" key="error-intakeYear">
                {error["intakeYear"]}
              </p>
            ) : null}
          </small>
...
<div className="col-12 md:col-6 field mt-5">
            ...
            <small className="p-error">
            {!_.isEmpty(error["month"]) ? (
              <p className="m-0" key="error-month">
                {error["month"]}
              </p>
            ) : null}
          </small>
...
<div className="col-12 md:col-6 field mt-5">
            ...
            <small className="p-error">
            {!_.isEmpty(error["campusLocation"]) ? (
              <p className="m-0" key="error-campusLocation">
                {error["campusLocation"]}
              </p>
            ) : null}
          </small>
            </div>
<div className="col-12 md:col-6 field mt-5">
            ...
            <small className="p-error">
            {!_.isEmpty(error["mode"]) ? (
              <p className="m-0" key="error-mode">
                {error["mode"]}
              </p>
            ) : null}
          </small>
