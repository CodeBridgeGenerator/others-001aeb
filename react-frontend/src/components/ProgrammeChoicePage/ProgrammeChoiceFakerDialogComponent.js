...
import programmeChoiceFakerFactory from "./programmeChoiceFakerFactory";
...
const ProgrammeChoiceFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = programmeChoiceFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("programmeChoice").create(elem));
...
export default connect(mapState, mapDispatch)(ProgrammeChoiceFakerDialogComponent);
