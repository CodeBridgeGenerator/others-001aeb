...
import seederData from "./programmeChoice.seeder.json";
...
const ProgrammeChoiceSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("programmeChoice").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(mapState, mapDispatch)(ProgrammeChoiceSeederDialogComponent);
