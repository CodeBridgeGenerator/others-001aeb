...
import seederData from "./parentsDetails.seeder.json";
...
const ParentsDetailsSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("parentsDetails").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(mapState, mapDispatch)(ParentsDetailsSeederDialogComponent);
