...
import parentsDetailsFakerFactory from "./parentsDetailsFakerFactory";
...
const ParentsDetailsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = parentsDetailsFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("parentsDetails").create(elem));
...
export default connect(mapState, mapDispatch)(ParentsDetailsFakerDialogComponent);
